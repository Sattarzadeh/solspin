...
export interface EventHandlerConfig<T> {
  ...
  handler: (payload: T, config: EventConfig, event: EventBridgeEvent<'event', EventBody<T>>, context: Context) => Promise<void>;
}

export function handleEvent<T>(config: EventHandlerConfig<T>): (event: SQSEvent, context: Context) => Promise<void> {
