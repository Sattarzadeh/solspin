...
export class TransactionService {
  ...
  public async processWithdrawal(
    ...
    const { transactionSignature, blockhash, lastValidBlockHeight }: BuildTransactionResponse =
      await this.blockchainService.buildTransaction(walletAddress, amount);

    // Broadcast transaction and verify it was successful
    const signature = await this.blockchainService.broadcastTransactionAndVerify(
  ...
  public async processDeposit(
    ...
    const transactionSerialized: Buffer = Buffer.from(base64Transaction, "base64");
    ...
    // Broadcast transaction and verify it was successful
    const transactionSignature = await this.blockchainService.broadcastTransactionAndVerify(
    ...
    const depositAmount = await this.blockchainService.getTransactionValueAndVerify(
