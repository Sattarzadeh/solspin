...
describe("BettingService", () => {
  ...
  describe("saveBet", () => {
    it("should throw an error if required fields are missing", async () => {
      await expect(saveBet("", "", 0, 0, "")).rejects.toThrow(InvalidInputError);
    });

    it("should throw an error if bet amount is invalid", async () => {
      await expect(saveBet("userId", "gameId", -1, 0, "WIN")).rejects.toThrow(InvalidInputError);
    });

    it("should throw an error if outcome is invalid", async () => {
      await expect(saveBet("userId", "gameId", 10, 20, "INVALID")).rejects.toThrow(
        InvalidInputError
      );
    });
    ...
    it("should create a transaction and record the bet", async () => {
      ...
      expect(recordBetSpy).toHaveBeenCalledWith("userId", 10, "WIN", 20, "gameId");
    });
  });
